...
== Installation and Getting Started with Development
    ...
    3. To run the app run the command <tt>rails s</tt> (for local dev) or <tt>rails s -p $PORT -b $IP</tt> (for c9 dev)
    4. To run tests run the commands <tt>rake spec</tt> for rsepc or <tt>rake cucumber</tt> for cucumber
